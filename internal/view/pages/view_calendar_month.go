...
// RenderMonthView renders the calendar grid showing one or more months.
func RenderMonthView(m *viewmodel.Model, t theme.Theme, height int) string {
	today := time.Now()
		...
		for week := 0; week < 6; week++ {
			...
			for wday := 0; wday < 7; wday++ {
				...
				for _, task := range m.Tasks {
					if task.TimeWindow.Start.Year() == cellDay.Year() &&
						task.TimeWindow.Start.Month() == cellDay.Month() &&
						task.TimeWindow.Start.Day() == cellDay.Day() {
						dailySP += task.StoryPoints
					}
				}
				...
				var dayColor lipgloss.TerminalColor = t.Muted
				if isCurrentMonth {
					dayColor = t.Fg
					if dailySP >= 9 {
						dayColor = t.P0Color
				...
				if isToday {
					...
					style = style.Foreground(t.FocusPurple).Bold(true)
				}
	...
	if len(selectedDayTasks) == 0 {
		...
		for _, task := range selectedDayTasks {
			isSelected := task.UUID == m.SelectedTaskUUID
			...
			pBadge := pStyle.Render(string(task.Priority))
			spText := lipgloss.NewStyle().Foreground(t.Muted).Render(fmt.Sprintf("%d SP", task.StoryPoints))
	...
	rightPanel := lipgloss.NewStyle().
		...
		Padding(1, 2).
		Render(
			lipgloss.NewStyle().Foreground(t.Accent).Bold(true).Render(fmt.Sprintf("📅 SELECTED DATE: %s", strings.ToUpper(m.SelectedDay.Format("Monday, Jun 2, 2006")))) + "\n" +
			lipgloss.NewStyle().Foreground(t.Muted).Render(strings.Repeat("─", availW)) + "\n\n" +
